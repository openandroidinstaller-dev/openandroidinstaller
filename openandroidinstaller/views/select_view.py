...
OpenAndroidInstaller works with the [TWRP recovery project](https://twrp.me/about) or [OrangeFox recovery](https://wiki.orangefox.tech/en/home), depending of your device.""",
        ...
        self.pick_super_empty_dialog = FilePicker(on_result=self.pick_super_empty_result)
...
Make sure the file is for **your exact phone model!**
    ...
    def pick_vbmeta_result(self, e: FilePickerResultEvent):
        path = ", ".join(map(lambda f: f.name, e.files)) if e.files else "Cancelled!"
        # update the textfield with the name of the file
        self.selected_vbmeta.value = (self.selected_vbmeta.value.split(":")[0] + f": {path}")
        if e.files:
    ...
    def pick_super_empty_result(self, e: FilePickerResultEvent):
        path = ", ".join(map(lambda f: f.name, e.files)) if e.files else "Cancelled!"
        # update the textfield with the name of the file
        self.selected_super_empty.value = (self.selected_super_empty.value.split(":")[0] + f": {path}")
        if e.files:
    ...
    def enable_button_if_ready(self, e):
        ...
        if (".zip" in self.selected_image.value) and (
            ...
            if (self.selected_dtbo.color and self.selected_dtbo.color == "red") or\
                    (self.selected_vbmeta.color and self.selected_vbmeta.color == "red") or\
                    (self.selected_super_empty.color and self.selected_super_empty.color == "red"):
                logger.error("Some additional images don't match. Please select different ones.")
                self.info_field.controls = [
                    Text(
