...
def recovery_works_with_device(supported_device_codes: [str], supported_recovery: [str], recovery_path: str) -> bool:
    ...
    for codename in supported_device_codes:
        if (codename in recovery_file_name) and ("twrp" in recovery_file_name or "TWRP" in recovery_file_name)\
                and ("twrp" in supported_recovery or not supported_recovery):
            logger.success("Selected recovery supported for this device.")
            return True
        elif recovery_file_name == "recovery.img" and ("orangefox" in supported_recovery):
